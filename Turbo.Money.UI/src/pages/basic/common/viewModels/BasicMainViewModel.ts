...
export default function BasicMainViewModel(
    ...
    const [modeItem, setModeItem] = useState<IModelItem | null>(null);
    ...
    const selectItem = (item: IModelItem | null | undefined) => {
        if (!item) {
            setModeItemId(null);
    ...
    const onAddSubmitted = async () => {
        ...
        if (errors.isError(createdItem))
            return;
        const validItem = createdItem as IModelItem;
    ...
    const onEditSubmitted = async () => {
        ...
        if (errors.isError(updatedItem))
            return;
        const validItem = updatedItem as IModelItem;
    ...
    const onDeleteSubmitted = async () => {
        ...
        if (errors.isError(deletedItem))
            return;
        const validItem = deletedItem as IModelItem;
    ...
    let modeViewModel: IBasicModeViewModel;
    switch (mode) {
        case "details":
